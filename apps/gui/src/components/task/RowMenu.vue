...
/** When `downloading`, the primary action is Pause; Resume only for paused/pending */
const isDownloading = () => props.task.status === 'downloading'
const canResume = () => props.task.status === 'paused' || props.task.status === 'pending'
const canRetry = () =>
  props.task.status === 'failed' || props.task.status === 'paused' || props.task.status === 'completed'
  ...
  <Teleport to="body">
    <Transition name="row-menu">
      ...
      >
        ...
        <button v-else-if="canResume()" class="row-menu-item" role="menuitem" type="button" @click="run('resume')">
          <svg
            class="row-menu-icon"
            width="14"
            height="14"
        ...
        <!-- Retry -->
        <button v-if="canRetry()" class="row-menu-item" role="menuitem" type="button" @click="run('retry')">
          <svg
            class="row-menu-icon"
            width="14"
            height="14"
