...
static BTDIG_MAGNET_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"class="torrent_magnet"[^>]*>\s*<a[^>]*href="(magnet:[^"]+)""#)
        .unwrap_or_else(|e| {
            log::error!("Regex compile failed: {e}");
            regex::Regex::new("").unwrap()
        })
});
static BTDIG_TITLE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"class="torrent_name"[^>]*>([^<]+)"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static BTDIG_SIZE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"class="torrent_size"[^>]*>([^<]+)"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static BTDIG_SEEDER_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"(\d+)\s*seeds?"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static BTDIG_LEECHER_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"(\d+)\s*leech?"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
// Mikan
static MIKAN_ITEM_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<item>([\s\S]*?)</item>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static MIKAN_TITLE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<title>([^<]+)</title>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static MIKAN_ENCLOSURE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<enclosure[^>]*url="([^"]+)"[^>]*length="(\d+)""#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static MIKAN_MAGNET_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"(magnet:\?[\\<\s"]+)"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
// 1337x
static X1337_ROW_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<tr>([\s\S]*?)</tr>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static X1337_TITLE_LINK_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<a[^>]*href="(/torrent/[^"]+)"[^>]*>(.*?)</a>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static X1337_SEEDER_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<td[^>]*class="seeds?"[^>]*>(\d+)</td>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static X1337_LEECHER_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<td[^>]*class="leeches?"[^>]*>(\d+)</td>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static X1337_SIZE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<td[^>]*class="size"[^>]*>([\s\S]*?)</td>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
static X1337_DETAIL_MAGNET_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"href="(magnet:\?xt=urn:btih:[a-fA-F0-9]{40}[^"]*)""#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
static NYAA_TITLE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<a[^>]*href="(/view/[^"]+)"[^>]*title="([^"]*)"[^>]*>"#).unwrap_or_else(
        |e| {
            log::error!("Regex compile failed: {e}");
            regex::Regex::new("").unwrap()
        },
...
static NYAA_TITLE_FALLBACK_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<a[^>]*href="(/view/[^"]+)"[^>]*>([\s\S]*?)</a>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static NYAA_MAGNET_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<a[^>]*href="(magnet:\?[^"]+)"[^>]*>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
static NYAA_NUM_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<td[^>]*class="text-center"[^>]*>(\d+)</td>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
static TG_TITLE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"<a[^>]*href="(/torrent/[^"]+)"[^>]*>([\s\S]*?)</a>"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
static TG_SIZE_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"Size:\s*([\d.]+\s*[KMGT]?i?B)"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static TG_SEEDER_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"(?:Seeders?|S(?:hd)?):\s*(\d+)"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
static TG_LEECHER_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r#"(?:Leechers?|L(?:hd)?):\s*(\d+)"#).unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
// Shared helpers
static STRIP_TAGS_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r"<[^>]*>").unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
...
static PARSE_SIZE_UNIT_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r"^([\d.]+)\s*([KMGT]?I?B|BYTES?)$").unwrap_or_else(|e| {
        log::error!("Regex compile failed: {e}");
        regex::Regex::new("").unwrap()
    })
});
