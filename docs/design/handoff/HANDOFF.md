...
This is a **deliberate inversion** of the original spec's "chat-first" stance. The inversion was made in Round 2 after measuring that power users want to _see their queue_, not converse with it. The decision is final and lives in §2.1.
...
### 2.4 What is _not_ in this prototype
...
| **Row flash**                | 180ms primary-blue border + scale(1.005) on a task row the moment it is clicked, before the detail panel opens. Anchors the user's attention to _which_ row they just clicked. |
