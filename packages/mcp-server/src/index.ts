...
server.tool(
  ...
  async ({ query }) => {
    try {
      ...
      intent.search_keywords = keywordGenerator.generate(intent).slice(0, 5)
      ...
      const best = resultEvaluator.pickBest(results, intent)
      if (!best) {
        return {
          content: [{ type: 'text', text: 'No suitable match after ranking' }],
          isError: true,
        }
      }
...
server.tool(
  ...
  async ({ url }) => {
    try {
      const task = await queue.add(url, undefined, {
        dir: config.downloads.base_dir,
      })
      return {
        content: [
