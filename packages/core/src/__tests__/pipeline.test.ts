...
import type { DownloadIntent, SearchResult, Task, TaskStatus, FileEntry } from '../types.js'
...
describe('Pipeline error cases', () => {
  ...
  it('handles QueueManager.add failure from aria2', async () => {
    ...
    await expect(queue.add('magnet:?test', 'query')).rejects.toThrow('aria2 connection refused')
  })
})
