...
function makeTask(overrides?: Partial<Task>): Task {
  return {
    ...
    progress: 0,
    speed: { down: 0, up: 0 },
    files: [{ name: 'Inception.mkv', path: '/downloads/Inception.mkv', size: 12_000_000_000, completed: 0 }],
    subtitle: undefined,
...
function createMockAria2(overrides?: Partial<Record<string, ReturnType<typeof vi.fn>>>): Aria2Client {
...
describe('TaskDatabase', () => {
  ...
  describe('update', () => {
    ...
    it('clears subtitle by setting to undefined via update', () => {
      ...
      const task = makeTask({
        subtitle: { language: 'en', path: '/sub.srt', downloadUrl: 'https://example.com/sub.srt', source: 'shooter' },
      })
      db.insert(task)
...
describe('QueueManager integration', () => {
  ...
  it('add() propagates source query into the Task', async () => {
    ...
    expect(aria2.mapToTask).toHaveBeenCalledWith(expect.objectContaining({ gid: 'gid-mock-001' }), 'my NL query')
