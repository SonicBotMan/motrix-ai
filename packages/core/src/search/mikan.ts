...
export function parseMikanRss(xml: string): SearchResult[] {
  ...
  for (const item of items) {
    try {
      ...
      const linkMatch =
        item.match(/<link><!\[CDATA\[(magnet:\?[^\]]*?)\]\]><\/link>/) ??
...
export function parseAnimeTitle(title: string): {
  ...
  const epMatch =
    title.match(/\b(?:EP?|第)\s*(\d{1,4})(?:\s*(?:END|Fin|完))?/i) ??
  ...
  async search(keywords: string[], _intent: DownloadIntent): Promise<SearchResult[]> {
    ...
    for (const host of hosts) {
      ...
      try {
        ...
        // Empty feed — try next mirror instead of treating as final
        logger.warn(`empty RSS from ${host}, trying next domain`)
        continue
      } catch (err) {
