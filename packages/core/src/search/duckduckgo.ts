...
export function parseDuckDuckGoHtml(html: string): SearchResult[] {
  ...
  const magnets = rawMagnets.map((m) =>
    decodeHtmlEntities(m.replace(/&amp;/g, "&")),
  );
  ...
  if (magnets.length > 0) {
    for (let idx = 0; idx < magnets.length; idx++) {
      ...
      if (snippetIdx >= 0 && snippets[snippetIdx]) {
        const sizeMatch = snippets[snippetIdx].match(
          /([\d.]+)\s*(TB|GB|MB|KB|B)/i,
        );
        if (sizeMatch) {
      ...
      let seeders = 0;
      if (snippetIdx >= 0 && snippets[snippetIdx]) {
        const seederMatch = snippets[snippetIdx].match(
          /(\d+)\s*(?:seeders?|seeds?|peers?)/i,
        );
  ...
  for (let i = 1; i < hrefBlocks.length; i++) {
    ...
    if (
      /torrent|magnet|btih|piratebay|1337x|rarbg|nyaa|limetorrents|torrentgalaxy|eztv/i.test(
        lowerUrl,
      )
    ) {
